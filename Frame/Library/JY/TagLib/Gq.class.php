...
class Gq extends TagLib {
	...
	*/
	protected $tags   =  array( 
		// 标签定义： 定义标签中对应的属性
		'music'=>array('attr'=>'name,singer_id,album_id,genre_id,type_id,limit,order,sort,offset,length,mod','level'=>3),
     ...
     */
     ...
     public function _music($tag,$content) {
		...
		$limit 		= 	isset($tag['limit'])?$tag['limit']:'200';//设置显示条数		
		//$order 		= 	isset($tag['order'])?(isset($tag['sort']) ? $tag['order'].' '.$tag['sort'] : $tag['order'].' desc'):'';//排序方式	 
		$sid		= 	isset($tag['singer_id'])?'\'singer_id\'=>'.$tag['singer_id'].',':'';//所属歌手id的数据
		...
		$where      = "array(";
		if (!empty($sid)|| !empty($aid) || !empty($gid) || !empty($tid)){//判断查询条件id
				$ids =  $sid.$aid.$gid.$tid;	
        ...
        $parseStr   .= 	"foreach(\$_result as $".$key."=>\$".$name."):";
        $parseStr   .= 	'extract($'.$name.');';
        if ('Songs' == $name) { 
			$name = 'Play';
		}elseif ('AlbumType' == $name)  {
			$name = 'Album/type';
		}elseif ('SingerType' == $name)  {
			$name = 'Singer/type';
		}
        $parseStr   .= 	'$url=U(\''.$name.'/\'.$id);';
