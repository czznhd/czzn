...
class ConfigController extends AdminController {
    ...
    public function cache() {
		$set = include './Application/Home/Conf/cache_config.php';
		$this->assign('set',$set);
        $this->meta_title = '缓存静态';
        $this->display();
    }
	
	public function setConfig (){
		$path='./Application/Home/Conf/cache_config.php';			
		if(!is_writeable($path)) {
			$this->error('目录不可写！');

		}else{			
			if('0' == $_POST['HTML_CACHE_ON']){				
				$set = include $path;
				$html='index'.$set['HTML_FILE_SUFFIX'];
				if(file_exists($html)) unlink($html);				
				//$_POST['URL_MODEL'] = C('URL_MODEL');
			}
			/*else{
				$_POST['URL_MODEL'] = '2';
			}*/
			...
			foreach ($_POST as $k=>$v) {
				$pat = "/'{$k}'=>'.*?'/";
				$rep = "'{$k}'=>'{$v}'";			
				if ('HTML_CACHE_ON' == $k) {$v = (int)$v;  $pat = "/'{$k}'=>\d/"; $rep = "'{$k}'=>{$v}";}
				$info=preg_replace($pat,$rep,$info);
			}
